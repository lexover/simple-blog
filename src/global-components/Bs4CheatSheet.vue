            ...
            <template v-for="(item, index) in filtered">
                ...
                <template v-for="(row, i) in item.rows">
                  <tr :key="`row-${index}-${i}`">
                    ...
                    <td>
                      <pre v-highlightjs="row.command" class='table-code'><code class="python"></code></pre>
                    </td>
                    <td class="result">
                      {{ row.result }}
                    </td>
...
<script>
import { CONTENT } from '../cheat-sheets/bs4-cheatsheet.js'

export default {
  name: "Bs4CheatSheet",
  data () {
    return {
      columns: ['Описание', 'Код Python', 'Результат выполнения команды'],
      content: CONTENT,
      selected: null,
    }
  },
